        ...
        if (existingMember) {
            ...
            if (existingMember.status === 'withdrawn') {
                ...
                await existingMember.update({
                    status: group.currentParticipants >= group.maxParticipants ? 'pending' : 'approved',
                    joinedAt: group.currentParticipants >= group.maxParticipants ? null : new Date()
                });
                ...
                return { 
                    success: true, 
                    isWaitlist: group.currentParticipants >= group.maxParticipants,
                    message: group.currentParticipants >= group.maxParticipants ? '已加入候補名單' : '報名成功！'
                };
            }
        }
...
async function rateGroup(groupId, userId, rating, review = '') {
    try {
        ...
        // 更新揪團平均評分
        const allRatings = await GroupMember.findAll({
            where: { groupId, rating: { [Op.ne]: null } },
            attributes: ['rating']
        });

        if (allRatings.length > 0) {
            const avgRating = allRatings.reduce((sum, m) => sum + m.rating, 0) / allRatings.length;
            await Group.update(
                { averageRating: avgRating, totalRatings: allRatings.length },
                { where: { id: groupId } }
            );
        }
